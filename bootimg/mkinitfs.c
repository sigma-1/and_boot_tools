...
static void _eject(struct stat *s, char *out, int olen, char *data, unsigned datasize)
{
    ...
    if(strlen(out) != (unsigned int)olen) die_mkinitfs("ACK!");
...
#ifndef WIN32
    d = opendir(in);
    if(d == 0) die_mkinitfs("cannot open directory '%s'", in);
#else
...
static void _archive(char *in, char *out, int ilen, int olen)
{
    ...
    if(lstat(in, &s)) die_mkinitfs("could not stat '%s'\n", in);
    ...
    if(S_ISREG(s.st_mode)){
        ...
        fd = open(in, O_RDONLY);
        if(fd < 0) die_mkinitfs("cannot open '%s' for read", in);

        tmp = (char*) malloc(s.st_size);
        if(tmp == 0) die_mkinitfs("cannot allocate %d bytes", s.st_size);
...
#endif
        if(size < 0) die_mkinitfs("cannot read symlink '%s'", in);
...
static void read_canned_config(char* filename)
{
    ...
    if (f == NULL) die_mkinitfs("failed to open canned file");
    ...
    while (fgets(line, CANNED_LINE_LENGTH, f) != NULL) {
        ...
        if (used >= allocated) {
            ...
            if (canned_config == NULL) die_mkinitfs("failed to reallocate memory");
        }
...
int main_mkinitfs(int argc, char *argv[])
{
    ...
    if(argc == 0) die_mkinitfs("no directories to process?!");
