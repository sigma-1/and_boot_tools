...
static char *cond_expr_to_str(struct policydb *pdb, struct cond_expr *expr)
{
	...
	for (curr = expr; curr != NULL; curr = curr->next) {
		if (curr->expr_type == COND_BOOL) {
			...
			default:
				sepol_log_err
				    ("Unknown conditional operator: %i",
				     curr->expr_type);
				goto exit;
			}
			...
			if (num_params == 2) {
				new_val =
				    create_str("(%s %s %s)", 3, val1, op, val2);
				free(val2);
			} else {
	...
	for (curr = expr; curr; curr = curr->next) {
		if (curr->expr_type == CEXPR_ATTR
			...
			case CEXPR_INCOMP:
				...
				break;
			default:
				sepol_log_err("Unknown constraint operator: %i",
					      curr->op);
				goto exit;
			}
			...
			default:
				sepol_log_err
				    ("Unknown constraint attribute: %i",
				     curr->attr);
				goto exit;
			}
			...
			if (curr->expr_type == CEXPR_ATTR) {
				...
				new_val =
				    create_str("%s %s %s", 3, attr1, op, names);
				free(names);
			}
		} else {
			uint32_t num_params;
					  ...
					  struct strs *non_mls_list)
{
	...
	for (curr = constraint_rules; curr != NULL; curr = curr->next) {
		...
		perms =
		    sepol_av_to_string(pdb, class->s.value, curr->permissions);
		if (strchr(perms, ' ')) {
			format_str = "%s %s { %s } %s;";
		} else {
			format_str = "%s %s %s %s";
...
static int class_validatetrans_rules_to_strs(struct policydb *pdb,
					     char *classkey,
					     struct constraint_node
					     *validatetrans_rules,
					     struct strs *mls_list,
					     struct strs *non_mls_list)
{
	...
	for (curr = validatetrans_rules; curr != NULL; curr = curr->next) {
		...
		rc = strs_create_and_add(strs, "%s %s %s;", 3, flavor, classkey,
					 expr);
		free(expr);
		if (rc != 0) {
			goto exit;
				       ...
				       struct strs *non_mls_strs)
{
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->validatetrans) {
			name = pdb->p_class_val_to_name[i];
			rc = class_validatetrans_rules_to_strs(pdb, name,
							       class->
							       validatetrans,
							       mls_strs,
							       non_mls_strs);
...
static int write_handle_unknown_to_conf(FILE * out, struct policydb *pdb)
{
	...
	case SEPOL_ALLOW_UNKNOWN:
		...
		break;
	default:
		sepol_log_err("Unknown value for handle-unknown: %i",
			      pdb->handle_unknown);
		return -1;
	}
...
static int write_class_and_common_rules_to_conf(FILE * out,
						struct policydb *pdb)
{
	class_datum_t *class;
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (!used[common->s.value - 1]) {
			perms =
			    class_or_common_perms_to_str(&common->permissions);
			if (!perms) {
				rc = -1;
				free(used);
				goto exit;
				      ...
				      class_datum_t * class)
{
	...
	case DEFAULT_TARGET:
		...
		break;
	default:
		sepol_log_err("Unknown default role value: %i",
			      class->default_user);
		return -1;
	}
				      ...
				      class_datum_t * class)
{
	...
	case DEFAULT_TARGET:
		...
		break;
	default:
		sepol_log_err("Unknown default type value: %i",
			      class->default_type);
		return -1;
	}
...
static int write_default_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->default_user != 0) {
			rc = write_default_user_to_conf(out,
							pdb->
							p_class_val_to_name[i],
							class);
			if (rc != 0) {
				goto exit;
			}
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->default_role != 0) {
			rc = write_default_role_to_conf(out,
							pdb->
							p_class_val_to_name[i],
							class);
			if (rc != 0) {
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->default_type != 0) {
			rc = write_default_type_to_conf(out,
							pdb->
							p_class_val_to_name[i],
							class);
			if (rc != 0) {
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->default_range != 0) {
			rc = write_default_range_to_conf(out,
							 pdb->
							 p_class_val_to_name[i],
							 class);
			if (rc != 0) {
...
static int write_sensitivity_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	if (num > 0) {
		sens_alias_map =
		    calloc(sizeof(*sens_alias_map), pdb->p_levels.nprim);
		if (!sens_alias_map) {
			rc = -1;
			goto exit;
		}
...
static int write_category_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	if (num > 0) {
		cat_alias_map =
		    calloc(sizeof(*cat_alias_map), pdb->p_cats.nprim);
		if (!cat_alias_map) {
			rc = -1;
			goto exit;
		}
...
static int write_level_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	for (i = 0; i < pdb->p_levels.nprim; i++) {
		...
		if (ebitmap_cardinality(&level->level->cat) > 0) {
			cats =
			    cats_ebitmap_to_str(&level->level->cat,
			       ...
			       avtab_datum_t * datum)
{
	...
	if (key->specified & AVTAB_AV) {
		...
		rule =
		    create_str("%s %s %s:%s %s;", 5, flavor, src, tgt, class,
			       perms);
	} else {
...
static int map_filename_trans_to_str(hashtab_key_t key, void *data, void *arg)
{
	...
	return strs_create_and_add(strs, "type_transition %s %s:%s %s \"%s\";",
...
static int write_role_decl_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (ebitmap_cardinality(&role->types.types) == 0)
			continue;
		types =
		    ebitmap_to_str(&role->types.types, pdb->p_type_val_to_name,
				   1);
		if (!types) {
			rc = -1;
			goto exit;
...
exit:
	...
	if (rc != 0) {
		sepol_log_err
		    ("Error writing role allow rules to policy.conf\n");
	}
...
exit:
	if (rc != 0) {
		sepol_log_err
		    ("Error writing user declarations to policy.conf\n");
	}
...
exit:
	...
	if (rc != 0) {
		sepol_log_err
		    ("Error writing sidcontext rules to policy.conf\n");
	}
...
static int write_selinux_fsuse_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	for (fsuse = pdb->ocontexts[5]; fsuse != NULL; fsuse = fsuse->next) {
		...
		case SECURITY_FS_USE_TASK:
			...
			break;
		default:
			sepol_log_err("Unknown fsuse behavior: %i",
				      fsuse->v.behavior);
			rc = -1;
			goto exit;
		}
...
static int write_selinux_port_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	for (portcon = pdb->ocontexts[2]; portcon != NULL;
		...
		case IPPROTO_SCTP:
			...
			break;
		default:
			sepol_log_err("Unknown portcon protocol: %i",
...
static int write_selinux_node_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	for (node = pdb->ocontexts[4]; node != NULL; node = node->next) {
		if (inet_ntop
		    (AF_INET, &node->u.node.addr, addr,
		     INET_ADDRSTRLEN) == NULL) {
			sepol_log_err("Nodecon address is invalid: %s",
				      strerror(errno));
			rc = -1;
			goto exit;
		}
		...
		if (inet_ntop
		    (AF_INET, &node->u.node.mask, mask,
		     INET_ADDRSTRLEN) == NULL) {
			sepol_log_err("Nodecon mask is invalid: %s",
				      strerror(errno));
			rc = -1;
			goto exit;
		}
...
static int write_selinux_node6_rules_to_conf(FILE * out, struct policydb *pdb)
{
	...
	for (node6 = pdb->ocontexts[6]; node6 != NULL; node6 = node6->next) {
		if (inet_ntop
		    (AF_INET6, &node6->u.node6.addr, addr,
		...
		if (inet_ntop
		    (AF_INET6, &node6->u.node6.mask, mask,
		     INET6_ADDRSTRLEN) == NULL) {
			sepol_log_err("Nodecon mask is invalid: %s",
				      strerror(errno));
			rc = -1;
