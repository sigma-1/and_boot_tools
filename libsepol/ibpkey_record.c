				      ...
				      uint64_t * subnet_prefix)
{
	...
	if (inet_pton(AF_INET6, subnet_prefix_str, &in_addr) <= 0) {
		ERR(handle,
		    "could not parse IPv6 address for ibpkey subnet prefix %s: %s",
			    ...
			    int low, int high, sepol_ibpkey_key_t ** key_ptr)
{
	...
	if (ibpkey_parse_subnet_prefix
	    (handle, subnet_prefix, &tmp_key->subnet_prefix) < 0)
		goto err;
				   ...
				   char **subnet_prefix)
{
	...
	if (ibpkey_expand_subnet_prefix
	    (handle, ibpkey->subnet_prefix, tmp_subnet_prefix) < 0)
		goto err;
