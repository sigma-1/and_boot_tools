...
static char *cond_expr_to_str(struct policydb *pdb, struct cond_expr *expr)
{
	...
	for (curr = expr; curr != NULL; curr = curr->next) {
		if (curr->expr_type == COND_BOOL) {
			...
			if (num_params == 2) {
				new_val =
				    create_str("(%s %s %s)", 3, op, val1, val2);
				free(val2);
			} else {
					  ...
					  struct strs *non_mls_list)
{
	...
	for (curr = constraint_rules; curr != NULL; curr = curr->next) {
		...
		rc = strs_create_and_add(strs, format_str, 3, classkey,
					 perms + 1, expr);
		free(expr);
		if (rc != 0) {
			goto exit;
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (name != NULL) {
			...
			if (!used[common->s.value - 1]) {
				perms =
				    class_or_common_perms_to_str(&common->
								 permissions);
				if (!perms) {
					rc = -1;
					free(perms);
...
static int write_default_rules_to_cil(FILE * out, struct policydb *pdb)
{
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->default_user != 0) {
			rc = write_default_user_to_cil(out,
						       pdb->
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->default_role != 0) {
			rc = write_default_role_to_cil(out,
						       pdb->
						       p_class_val_to_name[i],
						       class);
			if (rc != 0) {
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->default_type != 0) {
			rc = write_default_type_to_cil(out,
						       pdb->
						       p_class_val_to_name[i],
						       class);
			if (rc != 0) {
	...
	for (i = 0; i < pdb->p_classes.nprim; i++) {
		...
		if (class->default_range) {
			rc = write_default_range_to_cil(out,
							pdb->
							p_class_val_to_name[i],
							class);
			if (rc != 0) {
...
static size_t cats_ebitmap_len(struct ebitmap *cats, char **val_to_name)
{
	...
	ebitmap_for_each_bit(cats, node, i) {
		...
		if (range > 2) {
			len +=
			    strlen(val_to_name[i - 1]) + strlen("(range  ) ");
		} else if (range == 2) {
...
static int write_type_attribute_sets_to_cil(FILE * out, struct policydb *pdb)
{
	...
	for (i = 0; i < pdb->p_types.nprim; i++) {
		...
		if (ebitmap_cardinality(typemap) == 0)
			continue;
...
exit:
	...
	if (rc != 0) {
		sepol_log_err
		    ("Error writing filename typetransition rules to CIL\n");
	}
...
static int write_role_allow_rules_to_cil(FILE * out, struct policydb *pdb)
{
	...
	while (curr) {
		...
		rc = strs_create_and_add(strs, "(roleallow %s %s)", 2, role,
					 new);
		if (rc != 0) {
			goto exit;
		}
...
static int write_selinux_ibendport_rules_to_cil(FILE * out,
						struct policydb *pdb)
{
