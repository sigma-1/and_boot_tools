...
char *find_prop(const void *b, const size_t blen, const void *strings,
		const size_t slen, const char *prop)
{
	size_t i;
...
int main_dtbinfo(int argc, char **argv)
{
	...
	while (1) {
		p = mem_find(buffer, sizeof(buffer), dtb_magic,
			     sizeof(dtb_magic));
		if (p >= 0) {
	...
	for (i = 0; i < sizeof(dtb_props) / sizeof(dtb_props[0]); i++) {
		prop =
		    find_prop(buffer, sizeof(buffer), strings, sizeof(strings),
			      dtb_props[i]);
		if (prop)
			printf("DTB_%s=\"%s\"\n", dtb_props[i], prop);
	}
