...
static int stack_push(struct stack *stack, void *ptr)
{
	...
	if (stack->pos + 1 == stack->size) {
		new_stack =
		    realloc(stack->stack,
			    sizeof(*stack->stack) * (stack->size * 2));
		if (new_stack == NULL) {
			goto exit;
		}
			 ...
			 const struct class_perm_node *classperms)
{
	...
	for (classperm = classperms; classperm != NULL;
		...
		if (type & AVRULE_AV) {
			perms =
			    sepol_av_to_string(pdb, classperm->tclass,
					       classperm->data);
			if (perms == NULL) {
			...
			cil_println(indent, "(%s %s %s (%s (%s)))",
				    rule, src, tgt,
				    pdb->p_class_val_to_name[classperm->tclass -
							     1], perms + 1);
		} else {
			cil_println(indent, "(%s %s %s %s %s)",
				    rule, src, tgt,
				    pdb->p_class_val_to_name[classperm->tclass -
							     1],
				    pdb->p_type_val_to_name[classperm->data -
							    1]);
		}
	}
					    ...
					    char *type_name, int indent)
{
	...
	for (curr = role_list->head; curr != NULL; curr = curr->next) {
		role_node = curr->data;
		if (!is_id_in_ancestor_scope
		    (pdb, decl_stack, role_node->role_name, SYM_ROLES)) {
			continue;
		}
			      ...
			      struct list *attr_list)
{
	...
	for (avrule = avrule_list; avrule != NULL; avrule = avrule->next) {
		if ((avrule->
		     specified & (AVRULE_NEVERALLOW | AVRULE_XPERMS_NEVERALLOW))
			    ...
			    struct cond_expr *cond_expr, uint32_t flags)
{
	...
	for (curr = cond_expr; curr != NULL; curr = curr->next) {
		if (curr->expr_type == COND_BOOL) {
			...
			//          + null terminator
			len =
			    strlen(val1) + strlen(val2) + strlen(op) +
			    (num_params * 1) + 2 + 1;
			new_val = malloc(len);
			if (new_val == NULL) {
				log_err("Out of memory");
		...
		for (stype = 0; stype < num_stypes; stype++) {
			for (ttype = 0; ttype < num_ttypes; ttype++) {
				ebitmap_for_each_bit(&rule->tclasses, node, i) {
					...
					rc = semantic_level_to_cil(pdb, 1,
								   &rule->
								   trange.
								   level[0]);
					if (rc != 0) {
	...
	for (expr = exprs; expr != NULL; expr = expr->next) {
		if (expr->expr_type == CEXPR_ATTR
			...
			case CEXPR_INCOMP:
				...
				break;
			default:
				log_err("Unknown constraint operator type: %i",
					expr->op);
				rc = -1;
				goto exit;
			}
			...
			case CEXPR_L2H2:
				...
				break;
			default:
				log_err("Unknown expression attribute type: %i",
					expr->attr);
				rc = -1;
				goto exit;
			}
			...
			if (expr->expr_type == CEXPR_ATTR) {
				...
				len =
				    strlen(op) + strlen(attr1) + strlen(attr2) +
				    2 + 2 + 1;
				new_val = malloc(len);
				if (new_val == NULL) {
					log_err("Out of memory");
				...
				if (expr->attr & CEXPR_TYPE) {
					...
					rc = ebitmap_to_names(&expr->names,
							      pdb->
							      p_user_val_to_name,
							      &name_list,
							      &num_names);
				...
				rc = name_list_to_string(name_list, num_names,
							 &names);
				if (rc != 0) {
					goto exit;
				}
				...
				len =
				    strlen(op) + strlen(attr1) + strlen(names) +
				    2 + 2 + 1;
				new_val = malloc(len);
				if (new_val == NULL) {
					log_err("Out of memory");
			      ...
			      int is_constraint)
{
	...
	for (node = constraints; node != NULL; node = node->next) {
		...
		if (is_constraint) {
			perms =
			    sepol_av_to_string(pdb, class->s.value,
					       node->permissions);
			...
			void *datum, int scope)
{
	...
	if (class->default_user != 0) {
		...
		case DEFAULT_TARGET:
			...
			break;
		default:
			log_err("Unknown default user value: %i",
				class->default_user);
			rc = -1;
			goto exit;
		}
	...
	if (class->default_role != 0) {
		...
		case DEFAULT_TARGET:
			...
			break;
		default:
			log_err("Unknown default role value: %i",
				class->default_role);
			rc = -1;
			goto exit;
		}
	...
	if (class->default_type != 0) {
		...
		case DEFAULT_TARGET:
			...
			break;
		default:
			log_err("Unknown default type value: %i",
				class->default_type);
			rc = -1;
			goto exit;
		}
	...
	if (class->default_range != 0) {
		...
		case DEFAULT_TARGET_LOW_HIGH:
			...
			break;
		default:
			log_err("Unknown default range value: %i",
				class->default_range);
			rc = -1;
			goto exit;
		}
		       ...
		       int scope)
{
	...
	case ROLE_ROLE:
		...
		for (j = 0; j < num_types; j++) {
			if (is_id_in_scope
			    (pdb, decl_stack, types[j], SYM_TYPES)) {
		       ...
		       int scope)
{
	...
	if (scope == SCOPE_DECL) {
		if (!level->isalias) {
			...
			cil_println(indent, "(sensitivityaliasactual %s %s)",
				    key,
				    pdb->p_sens_val_to_name[level->level->sens -
							    1]);
		}
	}
					...
					struct ocontext *portcons)
{
	...
	for (portcon = portcons; portcon != NULL; portcon = portcon->next) {
		...
		case IPPROTO_SCTP:
			...
			break;
		default:
			log_err("Unknown portcon protocol: %i",
					...
					struct ocontext *nodes)
{
	...
	for (node = nodes; node != NULL; node = node->next) {
		if (inet_ntop
			...
			log_err("Nodecon address is invalid: %s",
				strerror(errno));
			rc = -1;
			goto exit;
		}
					 ...
					 struct ocontext *nodes)
{
	...
	for (node = nodes; node != NULL; node = node->next) {
		if (inet_ntop
		    (AF_INET6, &node->u.node6.addr, addr,
		...
		if (inet_ntop
		    (AF_INET6, &node->u.node6.mask, mask,
		     INET6_ADDRSTRLEN) == NULL) {
			log_err("Nodecon mask is invalid: %s", strerror(errno));
			rc = -1;
			goto exit;
					 ...
					 struct ocontext *fsuses)
{
	...
	for (fsuse = fsuses; fsuse != NULL; fsuse = fsuse->next) {
		...
		case SECURITY_FS_USE_TASK:
			...
			break;
		default:
			log_err("Unknown fsuse behavior: %i",
				fsuse->v.behavior);
			rc = -1;
			goto exit;
		}
...
static int ocontexts_to_cil(struct policydb *pdb)
{
	...
	static int (**ocon_funcs)(struct policydb * pdb,
...
static int user_extra_to_cil(struct sepol_module_package *mod_pkg)
{
	...
	while ((rc = get_line(&cur, end, &line)) > 0) {
		...
		matched =
		    tokenize(tmp, ' ', 4, &user_str, &user, &prefix_str,
			     &prefix);
		if (matched != 4) {
			rc = -1;
...
static int (*func_to_cil[SYM_NUM])(int indent, struct policydb * pdb,
				   struct avrule_block * block,
			       ...
			       struct stack *stack, int *indent)
{
	...
	if (block->flags & AVRULE_OPTIONAL) {
		while (stack->pos > 0) {
			...
			if (is_scope_superset
			    (&decl->required, &decl_tmp->required)) {
				break;
			}
...
static int handle_unknown_to_cil(struct policydb *pdb)
{
	...
	case SEPOL_ALLOW_UNKNOWN:
		...
		break;
	default:
		log_err("Unknown value for handle-unknown: %i",
			pdb->handle_unknown);
		rc = -1;
		goto exit;
	}
